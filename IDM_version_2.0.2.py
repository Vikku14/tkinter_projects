...
class Idm(Tk):
            ...
            try:
                ...
                if path == "":
                    file_check = os.path.isfile(
                        "/home/vikku/Downloads/VDM/{}.mp4".format(self.yt.title.strip()))
                else:
                    p = os.path.join(path, self.yt.title.strip())
    ...
    def openFile(self, Time, title, link_data, path):
    ...
    def complete(self, stream, file_handle):
        try:
            ...
            x = float(os.path.getsize(os.path.join(absolute_path, self.video_title) + ".mp4")
                      / (1024 * 1024))
            ...
            self.openFile(download_time, self.video_title, self.link_variable.get(), absolute_path)
            sleep(0.5)
            self.populate(self.frame2)
